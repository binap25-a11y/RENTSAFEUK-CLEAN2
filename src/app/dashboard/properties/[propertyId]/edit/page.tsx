...
// ##################################
// # 1. PRESENTATIONAL FORM COMPONENT
// ##################################
// This component's only job is to display the form with the data it receives.
// It is "dumb" and does not fetch data itself, preventing loops.

function PropertyEditForm({ initialData }: { initialData: PropertyFormValues }) {
    ...
    // The useForm hook is now initialized with the `values` prop.
    // This is the recommended, stable way to populate a form with async data.
    ...
    return (
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                {/* All the form fields are identical to the Add Property page for simplicity and stability */}
                <Card>
...
// ###############################
// This is the main component for the page. Its only job is to fetch data
// and handle loading/error states. It renders the form ONLY when data is ready.
...
export default function EditPropertyPage() {
    ...
    useEffect(() => {
        if (!firestore || !propertyId || !user) {
            setIsLoading(false);
            if (!user) setError("You must be logged in to edit a property.");
            return;
        }
        ...
        const fetchPropertyData = async () => {
            ...
            try {
                ...
                if (propertySnap.exists()) {
                    ...
                    // Sanitize the data to match the form schema exactly, providing default values
    ...
    if (!initialData) {
        return (
             <Card className="max-w-4xl mx-auto">
                <CardHeader><CardTitle className="text-destructive">Error</CardTitle></CardHeader>
                <CardContent>
                    <p>Property data could not be loaded. It may have been deleted.</p>
                     <Button asChild variant="link" className="mt-4 px-0">
                        <Link href="/dashboard/properties">Return to Properties</Link>
                    </Button>
                </CardContent>
